...
Evernote.BasicNote.prototype.getNoteUrl = function(shardUrl, shardId,
    searchWords, locale, includeNotebookGuid) {
  // /view.jsp?locale=default&shard=s1#v=t&n=8d3c87f9-6df9-4475-8ebc-45f336cf9757&b=0
  locale = locale || this.DEFAULT_LOCALE;
  shardUrl = shardUrl || "";
  var noteUrl = shardUrl.replace(/\/$/, "") + this.NOTE_URL_PREFIX + "locale="
      + locale + "&shard=" + shardId + "#v=t&n=" + this.guid;
  if (typeof searchWords == 'string' && searchWords.length > 0) {
    noteUrl += "&x=" + encodeURI(searchWords);
  } else {
    if (this.notebookGuid && includeNotebookGuid) {
      noteUrl += "&b=" + this.notebookGuid;
    }
  }
  return noteUrl;
};
...
(function() {
  ...
  Evernote.ClipProcessor = function ClipProcessor(path, size, interval,
    ...
    this.__definePositiveInteger__("syncGracePeriod",
        this.constructor.DEFAULT_SYNC_GRACE_PERIOD);
    var self = this;
    this.initialize(path, size, interval, gracePeriod, success, error);
  };
  ...
  Evernote.ClipProcessor.prototype.isSyncRequired = function() {
  ...
  Evernote.ClipProcessor.prototype.process = function(force) {
...
(function() {
  ...
  Evernote.ChromeExtension.prototype.startUp = function() {
    ...
    Evernote.context.removeAllAutoSavedNotes();
    ...
    if (note.guid) {
      noteItem.bind("click", {
        ...
        self.openNoteWindow(event.data.note.getNoteUrl(Evernote.context
            ...
            event.data.searchWords, Evernote.context.getLocale(), false));
      });
    }
    return noteItem;
  };
        ...
        .getSyncState(
            ...
            function(response, textStatus, xhr) {
              ...
              if (response.isResult()) {
                ...
                if (!Evernote.context.getSyncState(true)) {
...
(function() {
  ...
  Evernote.BingSearchHelperImpl.prototype.MESSAGE_ANCHOR_SELECTOR = "#results_area";
...
(function() {
  ...
  Evernote.ContentPreview.prototype.previewArticle = function(tabId, callback) {
    LOG.debug("ContentPreview.previewArticle");
    this.executeScript(tabId, {
      code : "Evernote.ContentPreviewScript.getInstance().previewArticle()"
    }, function() {
